...
- runs QuickJS on a worker thread while the Electrobun native event loop owns the main thread
...
The runtime is intentionally minimal right now. It embeds QuickJS-ng, supports relative ESM imports and async jobs, exposes `cottontail.args`, `cwd()`, `readFile()`, `writeFile()`, `env()`, `nanotime()`, basic fs mutation, platform/process info, and synchronous child process execution, and now has an early Electrobun bridge plus the first cottontail-owned Electrobun CLI path. It still does not expose a broad compatibility surface or a full Electrobun main-process API.
