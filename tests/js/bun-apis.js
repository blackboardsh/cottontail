...
assert(await fileSink.flush() === 7, "FileSink flush byte count mismatch");
assert(await fileSink.end() === 0, "FileSink end byte count mismatch");
assert(await Bun.file(sinkPath).text() === "one😋", "FileSink output mismatch");
const mutableFile = Bun.file(`${tmpDir}/mutable-file.txt`);
await mutableFile.write(new Blob(["mutable"]));
assert(await mutableFile.text() === "mutable", "Bun.file write mismatch");
assert((await mutableFile.stat()).size === 7, "Bun.file stat mismatch");
await mutableFile.unlink();
assert(!(await mutableFile.exists()), "Bun.file unlink mismatch");
assert(await new Blob(["bytes"]).stat() === undefined, "byte-backed Blob stat mismatch");
