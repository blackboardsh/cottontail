...
function validateCallCount(exact) {
  if (typeof exact !== "number") {
    throw invalidArgType("exact", "a number", exact);
  }
...
function copyTrackedFunctionProperties(source, target) {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    try {
      ...
      // Non-configurable function internals vary across engines; userland
      // properties are best-effort and should not make tracking fail.
    }
  }
...
export function fail(actual = undefined, expected = undefined, message = undefined, operator = undefined, stackStartFn = fail) {
...
function inspectForMessage(value) {
  try {
    const { inspect } = internalRequire("node:util");
...
function getPromiseResult(promiseFn, operator) {
...
export async function rejects(fn, expected, message) {
  const result = getPromiseResult(fn, "rejects");
...
export async function doesNotReject(fn, expected, message) {
  const result = getPromiseResult(fn, "doesNotReject");
  if (result.setupError) throw result.setupError;
  if (result.syncError) throw result.syncError;
  markPromiseHandled(result.promise);
  try {
