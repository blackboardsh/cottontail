...
// libuv linked into this executable. Fall back to the platform table retained
// by the JS compatibility layer when an older host lacks the native surface.
const nativeUvEntries = globalThis.cottontail?.uvErrorEntries?.();
export const uvErrorMap = Array.isArray(nativeUvEntries) && nativeUvEntries.length > 0
  ? new Map(nativeUvEntries.map(([errno, code, message]) => [errno, [code, message]]))
  : platformUvErrorMap;
...
const bindings = {
  ...
  uv: {
    ...
    ...Object.fromEntries([...uvErrorMap].map(([errno, [code]]) => [`UV_${code}`, errno])),
  },
  constants: {
    os: {
      UV_UDP_REUSEADDR: 4,
