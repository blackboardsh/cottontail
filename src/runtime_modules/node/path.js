// Faithful port of Node.js lib/path.js semantics.
...
function validateString(value, name) {
  if (typeof value !== "string") {
    ...
    // harness passes `DisposableString extends String` instances to join).
    if (value instanceof String) return String(value);
    // Message shape matches Bun's path implementation, which upstream tests
    // assert verbatim.
    const err = new TypeError(`The "${name}" property must be of type string, got ${typeof value}`);
...
function validateObject(value, name) {
  if (value === null || typeof value !== "object") {
    // Message shape matches Bun's path implementation, which upstream tests
    // assert verbatim.
    const err = new TypeError(`The "${name}" property must be of type object, got ${typeof value}`);
    err.code = "ERR_INVALID_ARG_TYPE";
    throw err;
  }
}
...
function normalizeString(path, allowAboveRoot, separator, isPathSeparatorFn) {
  ...
  for (let i = 0; i <= path.length; ++i) {
    ...
    else if (isPathSeparatorFn(code)) break;
...
function win32Cwd() {
  return cottontail.cwd().replace(/\//g, "\\");
}

function matchesGlobImpl(path, pattern, windows) {
  validateString(path, "path");
  validateString(pattern, "pattern");
  if (windows) {
    // On Windows backslashes are path separators (in both the path and the
    // pattern), not escapes; normalize to forward slashes before matching.
    path = path.replace(/\\/g, "/");
...
export const posix = {
  ...
  matchesGlob(path, pattern) {
    return matchesGlobImpl(path, pattern, false);
  },
...
export const win32 = {
  resolve(...args) {
    ...
    for (let i = args.length - 1; i >= -1; i--) {
      ...
      if (i >= 0) {
        ...
        // directories. We do not have those on this host; approximate by
  ...
  toNamespacedPath(path) {
    ...
    // Bun returns the resolved path here (Node returns the input unchanged).
    return resolvedPath;
  },
  ...
  matchesGlob(path, pattern) {
    return matchesGlobImpl(path, pattern, true);
  },
