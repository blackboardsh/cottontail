...
static JSValueRef ct_vm_context_create(
    ...
    static const char prepare_source[] =
        ...
        "}"
        "if(typeof g.Error==='function'&&!g.Error.__cottontailPreparedStack){"
            "const NativeError=g.Error;"
            "function Error(...args){"
                "const error=Reflect.construct(NativeError,args,new.target||Error);"
                "const raw=error.stack;"
                "if(typeof raw==='string'){"
                    "let computed=false,cached;"
                    "Object.defineProperty(error,'stack',{configurable:true,get(){"
                        "if(!computed){"
                            "computed=true;cached=raw;"
                            "const prepare=g.Error.prepareStackTrace;"
                            "if(typeof prepare==='function')cached=prepare(error,[]);"
                        "}"
                        "return cached;"
                    "},set(value){computed=true;cached=value}});"
                "}"
                "return error;"
            "}"
            "Object.setPrototypeOf(Error,NativeError);Error.prototype=NativeError.prototype;"
            "Object.defineProperty(Error,'__cottontailPreparedStack',{value:true});"
            "g.Error=Error;"
        "}"
        "})()";
    if (prepare_exception == NULL) {
