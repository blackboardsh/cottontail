...
pub fn parseWithTag(
    ...
    switch (tag) {
        ...
        .github => {
            const info = (hosted_git_info.HostedGitInfo.fromUrl(allocator, dependency) catch return null) orelse {
                return null;
            };
            defer info.deinit();
...
pub const Behavior = packed struct(u8) {
    ...
    pub inline fn add(this: Behavior, kind: anytype) Behavior {
        var new = this;
        @field(new, @tagName(kind)) = true;
        return new;
    }

    pub inline fn set(this: Behavior, kind: anytype, value: bool) Behavior {
        var new = this;
