...
fn LegacyWriter(comptime Context: type, comptime WriteError: type, comptime write_fn: anytype) type {
    return struct {
        context: Context,

        pub fn write(self: @This(), bytes: []const u8) WriteError!usize {
            return write_fn(self.context, bytes);
        }

        pub fn writeAll(self: @This(), bytes: []const u8) WriteError!void {
            _ = try write_fn(self.context, bytes);
        }
        ...
        pub fn print(self: @This(), comptime format: []const u8, args: anytype) WriteError!void {
            const bytes = std.fmt.allocPrint(std.heap.page_allocator, format, args) catch return error.OutOfMemory;
...
pub const Writer = LegacyWriter(*@This(), Allocator.Error, MutableString.writeAll);
...
pub const BufferedWriter = struct {
    ...
    pub const Writer = LegacyWriter(*BufferedWriter, Allocator.Error, BufferedWriter.writeAll);
