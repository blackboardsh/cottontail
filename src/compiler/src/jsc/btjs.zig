...
fn printSourceAtAddress(debug_info: *std.debug.SelfInfo, out_stream: *std.Io.Writer, address: usize, tty_config: std.io.tty.Config, fp: usize) !void {
    ...
    if (std.mem.startsWith(u8, symbol_info.name, "__")) {
        // Some module-program frames only happen to land inside the LLInt address range.
        allow_llint = false;
    }
    ...
    nosuspend {
        ...
        if (source_location) |sl| {
            if (printLineFromFile(out_stream, sl)) {
                if (sl.column > 0) {
                    // The caret itself occupies one character.
...
fn printLineFromFileAnyOs(out_stream: *std.Io.Writer, source_location: std.debug.SourceLocation) !void {
    ...
    // Always block here because this may run while event-loop code is crashing.
    ...
    if (std.mem.indexOfScalar(u8, slice, '\n')) |pos| {
        ...
        // Preserve a final newline when the source file ends without one.
