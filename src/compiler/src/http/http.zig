...
pub var socket_async_http_abort_tracker = bun.AutoArrayHashMap(u32, uws.AnySocket).init(bun.default_allocator);
...
noinline fn sendInitialRequestPayload(this: *HTTPClient, comptime is_first_call: bool, comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket) !InitialRequestPayloadResult {
    ...
    var temporary_send_buffer: std.Io.Writer.Allocating = .init(request_body_buffer.allocator());
    ...
    const headers_len = temporary_send_buffer.written().len;
    if (this.state.request_body.len > 0 and !this.flags.proxy_tunneling) {
        try writer.writeAll(this.state.request_body);
    }

    const to_send = temporary_send_buffer.written()[this.state.request_sent_len..];
    if (comptime Environment.allow_assert) {
    ...
    switch (this.state.request_stage) {
        ...
        .proxy_headers => {
            ...
            if (this.proxy_tunnel) |proxy| {
                ...
                var temporary_send_buffer: std.Io.Writer.Allocating = .init(stack_buffer.get());
                defer temporary_send_buffer.deinit();
                const writer = &temporary_send_buffer.writer;
                ...
                const headers_len = temporary_send_buffer.written().len;
                if (this.state.request_body.len > 0) {
                    writer.writeAll(this.state.request_body) catch {
                        this.closeAndFail(error.OutOfMemory, is_ssl, socket);
                        return;
                    };
                }
