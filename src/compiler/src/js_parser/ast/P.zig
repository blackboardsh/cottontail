    ...
    return struct {
        ...
        pub fn recordExport(noalias p: *P, loc: logger.Loc, alias: string, ref: Ref) !void {
            if (p.named_exports.get(alias)) |name| {
                // Import-equals pruning may rescan a part while converging.
                // Re-recording the exact same binding is idempotent; a second
                // location or symbol remains a duplicate-export error.
                if (name.alias_loc.eql(loc) and name.ref.asU64() == ref.asU64()) return;
            ...
            for (parts) |part| {
                for (part.stmts) |stmt| {
                    switch (stmt.data) {
                        ...
                        .s_local => |local| {
                            // cottontail: using-decls are never hoisted
                            if (local.kind.isUsing()) return true;
