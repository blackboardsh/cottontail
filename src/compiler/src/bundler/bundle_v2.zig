...
pub const Watcher = if (@hasDecl(bun.jsc, "hot_reloader"))
    bun.jsc.hot_reloader.NewHotReloader(BundleV2, EventLoop, true)
else
    void;
...
pub const BundleV2 = struct {
    ...
    resolve_tasks_waiting_for_import_source_index: std.AutoArrayHashMapUnmanaged(Index.Int, BabyList(struct { to_source_index: Index, import_record_index: u32 })) = .empty,
    ...
    pub fn init(
        ...
        this.transpiler.allocator = this.graph.heap.allocator();
        this.transpiler.resolver.allocator = this.graph.heap.allocator();
        this.transpiler.linker.allocator = this.graph.heap.allocator();
    ...
    pub fn allocator(this: *BundleV2) std.mem.Allocator {
    ...
    pub fn generateFromCLIReleasable(
        ...
        minify_duration.* = @as(u64, @intCast(@divTrunc(@as(i64, @truncate(bun.nanoTimestamp())) - @as(i64, @truncate(bun.cli.start_time)), @as(i64, std.time.ns_per_ms))));
    ...
    pub fn enqueueOnResolvePluginIfNeeded(
        ...
        if (comptime jsc.API.JSBundler.supports_plugins) {
            if (this.plugins) |plugins| if (plugins.hasAnyMatches(&import_record.path, false)) {
    ...
    pub fn enqueueEntryPointOnResolvePluginIfNeeded(
        ...
        if (comptime jsc.API.JSBundler.supports_plugins) {
            if (this.plugins) |plugins| plugin: {
                var temp_path = Fs.Path.init(entry_point);
                temp_path.namespace = "file";
                if (!plugins.hasAnyMatches(&temp_path, false)) break :plugin;
    ...
    pub fn enqueueOnLoadPluginIfNeededImpl(this: *BundleV2, parse: *ParseTask) bool {
        if (comptime jsc.API.JSBundler.supports_plugins) {
            if (this.plugins) |plugins| if (plugins.hasAnyMatches(&parse.path, true)) {
    ...
    pub fn onParseTaskComplete(parse_result: *ParseTask.Result, this: *BundleV2) void {
        ...
        if (this.bun_watcher) |watcher| {
            if (parse_result.watcher_data.fd.isValid()) {
                const source = switch (parse_result.value) {
...
pub const ExportData = struct {
    ...
    potentially_ambiguous_export_star_refs: BabyList(ImportData) = .empty,
...
pub fn generateUniqueKey() u64 {
    ...
    std.Io.randomSecure(std.Io.Threaded.global_single_threaded.io(), &random_bytes) catch bun.outOfMemory();
    const key = std.mem.readInt(u64, &random_bytes, .little) & @as(u64, 0x0FFFFFFF_FFFFFFFF);
