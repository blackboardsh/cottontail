pub noinline fn computeChunks(
    ...
    var css_chunks = bun.AutoArrayHashMap(u64, Chunk).init(temp_allocator);
    ...
    for (chunks, 0..) |*chunk, chunk_id| {
        ...
        if (chunk.template.needs(.dir)) {
            ...
            const dir_len = std.Io.Dir.cwd().realPathFile(io, dir_path, &real_path_buf) catch
                bun.path.normalizeBuf(dir_path, &real_path_buf, .auto).len;
            const dir = real_path_buf[0..dir_len];
