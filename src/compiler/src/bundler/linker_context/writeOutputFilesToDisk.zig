...
pub fn writeOutputFilesToDisk(
    ...
    var root_dir = std.Io.Dir.cwd().createDirPathOpen(io, root_path, .{}) catch |err| {
        if (err == error.NotDir) {
    ...
    for (chunks, 0..) |*chunk, chunk_index_in_chunks_list| {
        ...
        if (std.fs.path.dirnamePosix(rel_path)) |rel_parent| {
            if (rel_parent.len > 0) {
                root_dir.createDirPath(io, rel_parent) catch |err| {
        ...
        switch (chunk.content.sourcemap(c.options.source_maps)) {
            .external, .linked => |tag| {
                ...
                writeOutputFile(root_dir, io, source_map_final_rel_path, output_source_map, false) catch |err| {
                    try c.log.addErrorFmt(null, Logger.Loc.Empty, bun.default_allocator, "{s} writing sourcemap for chunk {f}", .{
                        @errorName(err),
                        bun.fmt.quote(chunk.final_rel_path),
                    });
            ...
            if (c.options.generate_bytecode_cache) {
                ...
                if (loader.isJavaScriptLike()) {
                    ...
                    if (jsc.CachedBytecode.generate(c.options.output_format, code_result.buffer, &source_provider_url)) |result| {
                        ...
                        writeOutputFile(root_dir, io, fdpath[0 .. chunk.final_rel_path.len + bun.bytecode_extension.len], bytecode, chunk.flags.is_executable) catch |err| {
                            c.log.addErrorFmt(null, Logger.Loc.Empty, bun.default_allocator, "{s} writing bytecode for chunk {f}", .{
                                @errorName(err),
                                bun.fmt.quote(chunk.final_rel_path),
                            }) catch unreachable;
                            return error.WriteFailed;
        ...
        writeOutputFile(root_dir, io, rel_path, code_result.buffer, chunk.flags.is_executable) catch |err| {
            try c.log.addErrorFmt(null, Logger.Loc.Empty, bun.default_allocator, "{s} writing chunk {f}", .{
                @errorName(err),
                bun.fmt.quote(chunk.final_rel_path),
            });
            return error.WriteFailed;
...

    {
        ...
        for (c.parse_graph.additional_output_files.items, additional_output_files) |*src, *dest| {
            ...
            writeOutputFile(root_dir, io, src.dest_path, bytes, false) catch |err| {
                c.log.addErrorFmt(null, Logger.Loc.Empty, bun.default_allocator, "{s} writing file {f}", .{
                    @errorName(err),
