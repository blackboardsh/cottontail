...
pub const ModuleInfo = struct {
    ...
    requested_modules: bun.AutoArrayHashMap(StringID, FetchParameters),
    ...
    fn init(allocator: std.mem.Allocator, is_typescript: bool) ModuleInfo {
        return .{
            ...
            .requested_modules = bun.AutoArrayHashMap(StringID, FetchParameters).init(allocator),
            .buffer = .empty,
            .record_kinds = .empty,
    ...
    pub fn finalize(self: *ModuleInfo) !void {
        bun.assert(!self.finalized);
        var local_name_to_module_name = bun.AutoArrayHashMap(StringID, struct { module_name: StringID, import_name: StringID, record_kinds_idx: usize, is_namespace: bool }).init(bun.default_allocator);
