...
_socket: Socket = .{ .SocketTCP = .{ .socket = .{ .detached = {} } } },
status: ConnectionState = .disconnected,

_write_buffer: bun.OffsetByteList = .{},
_read_buffer: bun.OffsetByteList = .{},
_last_message_start: u32 = 0,
_sequence_id: u8 = 0,
...
_server_version: bun.ByteList = .{},
_connection_id: u32 = 0,
_capabilities: Capabilities = .{},
_character_set: CharacterSet = CharacterSet.default,
_status_flags: StatusFlags = .{},

_auth_plugin: ?AuthMethod = null,

_auth_data: std.array_list.Managed(u8) = std.array_list.Managed(u8).init(bun.default_allocator),
_database: []const u8 = "",
_user: []const u8 = "",
_password: []const u8 = "",
_options: []const u8 = "",
_options_buf: []const u8 = "",
...
pub inline fn isAbleToWrite(this: *const @This()) bool {
    return this.status == .connected and
        !this._flags.has_backpressure and
        this._write_buffer.len() < MAX_PIPELINE_SIZE;
}

pub inline fn isProcessingData(this: *@This()) bool {
    return this._flags.is_processing_data;
}
pub inline fn hasBackpressure(this: *const @This()) bool {
    return this._flags.has_backpressure;
}
pub inline fn resetBackpressure(this: *@This()) void {
    this._flags.has_backpressure = false;
}
pub inline fn canFlush(this: *const @This()) bool {
    return !this._flags.has_backpressure and // if has backpressure we need to wait for onWritable event
        this.status == .connected and //and we need to be connected
        // we need data to send
        (this._write_buffer.len() > 0 or
...
pub inline fn isIdle(this: *const @This()) bool {
    return this.queue.current() == null and this._write_buffer.len() == 0;
}
...
pub fn flushQueue(this: *@This()) error{AuthenticationFailed}!void {
    this.flushData();
    if (!this._flags.has_backpressure) {
...
fn flushData(this: *@This()) void {
    ...
    const chunk = this._write_buffer.remaining();
    if (chunk.len == 0) {
        return;
    }

    const wrote = this._socket.write(chunk);
...
pub fn cleanup(this: *MySQLConnection) void {
    ...
    var write_buffer = this._write_buffer;
    var read_buffer = this._read_buffer;
    var statements = this.statements;
    var tls_config = this._tls_config;
    const options_buf = this._options_buf;
    this._write_buffer = .{};
    this._read_buffer = .{};
...
pub fn doHandshake(this: *MySQLConnection, success: i32, ssl_error: uws.us_bun_verify_error_t) !bool {
    debug("onHandshake: {d} {d} {s}", .{ success, ssl_error.error_no, @tagName(this._ssl_mode) });
    const handshake_success = if (success == 1) true else false;
    this._sequence_id = this._sequence_id +% 1;
    if (handshake_success) {
        this._tls_status = .ssl_ok;
        if (this._tls_config.reject_unauthorized != 0) {
...
pub fn readAndProcessData(this: *MySQLConnection, data: []const u8) !void {
    this._flags.is_processing_data = true;
    defer this._flags.is_processing_data = false;
    // Clear the timeout.
    this._socket.setTimeout(0);

    SocketMonitor.read(data);

    if (this._read_buffer.remaining().len == 0) {
        ...
        this.processPackets(StackReader, reader) catch |err| {
            ...
            if (err == error.ShortRead) {
                ...
                this._last_message_start = 0;
                this._read_buffer.byte_list.len = 0;
                this._read_buffer.write(bun.default_allocator, data[offset..]) catch @panic("failed to write to read buffer");
            } else {
                if (comptime bun.Environment.allow_assert) {
    ...
    {
        this._read_buffer.head = this._last_message_start;

        this._read_buffer.write(bun.default_allocator, data) catch @panic("failed to write to read buffer");
        this.processPackets(Reader, this.bufferedReader()) catch |err| {
            ...
            if (comptime bun.Environment.allow_assert) {
                debug("Received short read: last_message_start: {d}, head: {d}, len: {d}", .{
                    this._last_message_start,
                    this._read_buffer.head,
                    this._read_buffer.byte_list.len,
                });
            }
...
pub fn processPackets(this: *MySQLConnection, comptime Context: type, reader: NewReader(Context)) AnyMySQLError.Error!void {
    while (true) {
        ...
        debug("sequence_id: {d} header: {d}", .{ this._sequence_id, header_length });
        ...
        // Update sequence id
        this._sequence_id = header.sequence_id +% 1;
...
pub fn handleHandshake(this: *MySQLConnection, comptime Context: type, reader: NewReader(Context)) AnyMySQLError.Error!void {
    ...
    // Store server info
    this._server_version = try handshake.server_version.toOwned();
    this._connection_id = handshake.connection_id;
    ...
    this._capabilities = Capabilities.getDefaultCapabilities(this._ssl_mode != .disable, this._database.len > 0)
        .intersect(handshake.capability_flags);

    // Override with utf8mb4 instead of using server's default
    this._character_set = CharacterSet.default;
    this._status_flags = handshake.status_flags;
    ...
    debug(
        ...
        \\
    , .{
        this._server_version.slice(),
        this._connection_id,
        this._character_set,
        this._character_set.label(),
    ...
    this._auth_data.clearAndFree();

    // Store auth data
    try this._auth_data.ensureTotalCapacity(handshake.auth_plugin_data_part_1.len + handshake.auth_plugin_data_part_2.len);
    try this._auth_data.appendSlice(handshake.auth_plugin_data_part_1[0..]);
    try this._auth_data.appendSlice(handshake.auth_plugin_data_part_2[0..]);

    // Get auth plugin
    if (handshake.auth_plugin_name.slice().len > 0) {
        this._auth_plugin = AuthMethod.fromString(handshake.auth_plugin_name.slice()) orelse {
    ...
    if (this._capabilities.CLIENT_SSL) {
        var response = SSLRequest{
            .capability_flags = this._capabilities,
        ...
        this._capabilities = response.capability_flags;
        this._tls_status = .message_sent;
        this.flushData();
        if (!this._flags.has_backpressure) {
            try this.upgradeToTLS();
        }
...
fn handleHandshakeDecodePublicKey(this: *MySQLConnection, comptime Context: type, reader: NewReader(Context)) !void {
    ...
    var encrypted_password = Auth.caching_sha2_password.EncryptedPassword{
        ...
        .nonce = this._auth_data.items,
        .sequence_id = this._sequence_id,
    };
...
pub fn handleAuth(this: *MySQLConnection, comptime Context: type, reader: NewReader(Context), header_length: u24) !void {
    ...
    switch (first_byte) {
        @intFromEnum(PacketType.OK) => {
            ...
            this._status_flags = ok.status_flags;
            this._flags.is_ready_for_query = true;
                        ...
                        switch (response.status) {
                            ...
                            .continue_auth => {
                                ...
                                if (this._ssl_mode == .disable) {
                                    ...
                                    var packet = try this.writer().start(this._sequence_id);
                                    ...
                                    var packet = try this.writer().start(this._sequence_id);
                                    try this.writer().writeZ(this._password);
                                    try packet.end();
                                    this.flushData();
                                }
                            },
        ...
        PacketType.AUTH_SWITCH => {
            ...
            this._auth_plugin = auth_method;
            this._auth_data.clearRetainingCapacity();
            try this._auth_data.appendSlice(auth_data);
...
pub fn sendHandshakeResponse(this: *MySQLConnection) AnyMySQLError.Error!void {
    ...
    if (this._auth_plugin) |plugin| {
        ...
        if (requires_password and this._password.len == 0) {
    ...
    var response = HandshakeResponse41{
        ...
        .max_packet_size = 0, //16777216,
        .character_set = CharacterSet.default,
        .username = .{ .temporary = this._user },
        .database = .{ .temporary = this._database },
        .auth_plugin_name = .{
            .temporary = if (this._auth_plugin) |plugin|
                switch (plugin) {
    ...
    if (this._auth_plugin) |plugin| {
        if (this._auth_data.items.len == 0) {
            return error.MissingAuthData;
        }

        response.auth_response = .{ .temporary = try plugin.scramble(this._password, this._auth_data.items, &scrambled_buf) };
    }
...
pub fn sendAuthSwitchResponse(this: *MySQLConnection, auth_method: AuthMethod, plugin_data: []const u8) !void {
    ...
    response.auth_response = .{
        .temporary = try auth_method.scramble(this._password, plugin_data, &scrambled_buf),
    };

    var response_writer = this.writer();
    var packet = try response_writer.start(this._sequence_id);
...
pub const Writer = struct {
    ...
    pub fn offset(this: Writer) usize {
        return this.connection._write_buffer.len();
    }
};
...
pub const Reader = struct {
    ...
    pub fn markMessageStart(this: Reader) void {
        this.connection._last_message_start = this.connection._read_buffer.head;
    }
    ...
    pub fn skip(this: Reader, count: isize) void {
        if (count < 0) {
            ...
            if (abs_count > this.connection._read_buffer.head) {
    ...
    pub fn ensureCapacity(this: Reader, count: usize) bool {
        return this.connection._read_buffer.remaining().len >= count;
    }
...
fn checkIfPreparedStatementIsDone(this: *MySQLConnection, statement: *MySQLStatement) void {
    ...
    if (statement.columns_received == statement.columns.len and statement.params_received == statement.params.len) {
        ...
        this._flags.waiting_to_prepare = false;
        this._flags.is_ready_for_query = true;
        this.queue.markAsReadyForQuery();
...
pub fn handlePreparedStatement(this: *MySQLConnection, comptime Context: type, reader: NewReader(Context), header_length: u24) !void {
    ...
    if (statement.statement_id > 0) {
        ...
        if (!this._capabilities.CLIENT_DEPRECATE_EOF and header_length < 0xFFFFFF and @as(PacketType, @enumFromInt(first_byte)) == .EOF) {
            var eof = EOFPacket{};
        ...
        if (this._capabilities.CLIENT_DEPRECATE_EOF) {
            this.checkIfPreparedStatementIsDone(statement);
        }
        return;
    }
...
fn handleResultSetOK(this: *MySQLConnection, request: *JSMySQLQuery, statement: *MySQLStatement, status_flags: StatusFlags, last_insert_id: u64, affected_rows: u64) void {
    this._status_flags = status_flags;
    ...
    this._flags.is_ready_for_query = is_last_result;
    if (is_last_result) {
        this.queue.markAsReadyForQuery();
        this.queue.markCurrentRequestAsFinished(request);
    }
