...
pub const Loader = struct {
    ...
    pub fn loadProcess(this: *Loader) !void {
        ...
        const process_environment: std.process.Environ = .{ .block = if (Environment.isWindows)
            .global
        else
            .{ .slice = std.mem.span(std.c.environ) } };
        var environment = try process_environment.createMap(this.allocator);
        defer environment.deinit();

        try this.map.map.ensureTotalCapacity(environment.keys().len);
        for (environment.keys(), environment.values()) |key, value| {
            try this.map.putAllocKeyAndValue(this.allocator, key, value);
        }
    ...
    fn loadDefaultFiles(
        ...
        const dir_handle = std.Io.Dir.cwd();
    ...
    pub fn loadEnvFile(
        ...
        var buf = dir.readFileAlloc(
            std.Io.Threaded.global_single_threaded.io(),
            base,
            this.allocator,
            .unlimited,
        ) catch |err| {
            if (!this.quiet and err != error.FileNotFound) {
                Output.prettyErrorln("<r><red>{s}<r> error loading {s} file", .{ @errorName(err), base });
            }
            @field(this, base) = logger.Source.initPathString(base, "");
            return;
        };
        const amount_read = buf.len;
        buf = try this.allocator.realloc(buf, amount_read + 1);
    ...
    pub fn loadEnvFileDynamic(
        ...
        var buf = std.Io.Dir.cwd().readFileAlloc(
            std.Io.Threaded.global_single_threaded.io(),
            file_path,
            this.allocator,
            .unlimited,
...
pub const Map = struct {
    ...
    pub fn cloneWithAllocator(this: *const Map, new_allocator: std.mem.Allocator) OOM!Map {
        var result = Map.init(new_allocator);
        try result.map.ensureTotalCapacity(this.map.count());
        var map_iterator = this.map.iterator();
        while (map_iterator.next()) |entry| {
            const key = try new_allocator.dupe(u8, entry.key_ptr.*);
            const value = try new_allocator.dupe(u8, entry.value_ptr.value);
            result.map.putAssumeCapacity(key, .{
                .value = value,
                .conditional = entry.value_ptr.conditional,
            });
        }
        return result;
    }
