            ...
            if (info.tag_type) |UnionTagType| {
                inline for (info.fields) |u_field| {
                    if (value == @field(UnionTagType, u_field.name)) {
                        var properties = try BabyList(js_ast.G.Property).initCapacity(allocator, 1);
                        properties.appendAssumeCapacity(G.Property{
                            .key = Expr.init(E.String, E.String{ .data = u_field.name }, logger.Loc.Empty),
                            .value = try toAST(allocator, u_field.type, @field(value, u_field.name)),
                        });
                        return Expr.init(
                            E.Object,
                            E.Object{ .properties = properties, .is_single_line = true },
                            logger.Loc.Empty,
                        );
                    }
                }
