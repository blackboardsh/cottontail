...
pub fn NewWriter(
    ...
    return struct {
        ...
        pub const StdWriter = struct {
            ...
            pub fn writeAll(self: StdWriter, bytes: anytype) error{}!void {
                _ = self.inner.writeAll(bytes) catch unreachable;
            }
        };
...
pub fn printAst(
    ...
    if (opts.runtime_transpiler_cache) |cache| {
        var srlz_res: std.Io.Writer.Allocating = .init(bun.default_allocator);
        defer srlz_res.deinit();
        if (have_module_info) try opts.module_info.?.asDeserialized().serialize(&srlz_res.writer);
        cache.put(printer.writer.ctx.getWritten(), if (source_maps_chunk) |chunk| chunk.buffer.list.items else "", srlz_res.written());
    }
...
pub fn serializeModuleInfo(module_info: ?*analyze_transpiled_module.ModuleInfo) ?[]const u8 {
    ...
    var output = std.Io.Writer.Allocating.init(bun.default_allocator);
    defer output.deinit();
    deserialized.serialize(&output.writer) catch return null;
