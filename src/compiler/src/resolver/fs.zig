...
pub const FileSystem = struct {
    ...
    pub fn setTopLevelDir(this: *FileSystem, value: stringZ) !void {
        ...
        this.top_level_dir_owned = owned;
        this.top_level_dir = owned;
        this.fs.cwd = owned;
    }
    ...
    pub fn tmpname(extname: string, buf: []u8, hash: u64) std.fmt.BufPrintError![:0]u8 {
        const hex_value = @as(u64, @truncate(@as(u128, @intCast(hash)) | @as(u128, @intCast(bun.nanoTimestamp()))));
    ...
    pub const RealFS = struct {
        ...
        var get_platform_tempdir = bun.once(platformTempDirImpl);
        ...
        pub fn kind(
            ...
            const io = std.Io.Threaded.global_single_threaded.io();
            const stat = try std.Io.Dir.cwd().statFile(io, absolute_path_c, .{ .follow_symlinks = false });
