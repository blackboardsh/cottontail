...
pub const Resolver = struct {
    ...
    runtime_aliases: []const RuntimeAlias = &.{},
    ...
    pub const RuntimeAlias = struct {
        specifier: []const u8,
        path: []const u8,
    };

    pub fn runtimeAlias(r: *const Resolver, specifier: []const u8) ?[]const u8 {
        for (r.runtime_aliases) |alias| {
            if (strings.eqlLong(alias.specifier, specifier, true)) return alias.path;
        }
        return null;
    }
                ...
                if (entries.get(path.name.filename)) |query| {
                    ...
                    if (symlink_path.len > 0) {
                        ...
                        if (!query.entry.cache.fd.isValid() and store_fd) {
                            ...
                            var file: bun.FD = .fromStdFile(try std.Io.Dir.openFileAbsolute(
    ...
    pub fn loadNodeModules(
        ...
        // Auto-install is a separate package-manager integration. The embedded
        // compiler still performs normal node_modules resolution when disabled.
        if (comptime bun.enable_package_manager) {
            ...
            append_bin_dir: {
                if (info.hasNodeModules()) {
                    if (entries.hasComptimeQuery("node_modules")) {
                        ...
                        const directory = this_dir.openDir(io, "node_modules/.bin", .{}) catch
                            break :append_bin_dir;
                        defer directory.close(io);
                        const bin_path_len = directory.realPath(io, bufs(.node_bin_path)) catch break :append_bin_dir;
                        const bin_path = bufs(.node_bin_path)[0..bin_path_len];
                ...
                if (info.isNodeModules()) {
                    if (entries.getComptimeQuery(".bin")) |q| {
                        if (q.entry.kind(rfs, r.store_fd) == .dir) {
                            ...
                            const this_dir = fd.stdDir();
                            const io = std.Io.Threaded.global_single_threaded.io();
                            const directory = this_dir.openDir(io, ".bin", .{}) catch break :append_bin_dir;
                            defer directory.close(io);
                            const bin_path_len = directory.realPath(io, bufs(.node_bin_path)) catch break :append_bin_dir;
                            const bin_path = bufs(.node_bin_path)[0..bin_path_len];
                            bin_folders_lock.lock();
