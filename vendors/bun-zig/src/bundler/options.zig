...
pub fn openOutputDir(output_dir: string) !std.Io.Dir {
    return std.Io.Dir.cwd().openDir(output_dir, .{}) catch brk: {
        std.Io.Dir.cwd().makeDir(output_dir) catch |err| {
            Output.printErrorln("error: Unable to mkdir \"{s}\": \"{s}\"", .{ output_dir, @errorName(err) });
            Global.crash();
        };

        const handle = std.Io.Dir.cwd().openDir(output_dir, .{}) catch |err2| {
